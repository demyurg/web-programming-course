...
// Типы для операций калькулятора
...
type UserCriteria = string | number | Partial<User>;
...
function findUser(users: User[], criteria: UserCriteria): User | undefined {
    ...
    if (typeof criteria === 'object') {
        return users.find(user => {
            return Object.keys(criteria).every(key => {
                ...
                return user[userKey] === (criteria as any)[key];
            });
        });
    }
    return undefined;
