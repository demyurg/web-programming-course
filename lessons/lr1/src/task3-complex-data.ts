...
function getCourseStats(course: Course, students: Student[]): CourseStats {
    ...
    const studentsWithGrades = enrolledStudents.filter(student => {
        const grades = student.grades[course.id];
...
function getTopStudents(students: Student[], courseId: number, limit: number): Student[] {
    return students
        .map(student => ({
            ...student,
...
if (student1 && student2 && course1 && course2) {
    enrollStudent(student1, course1);
    enrollStudent(student2, course1);
    enrollStudent(student1, course2);

    // Выставляем оценки
    assignGrade(student1, 101, 95);
    assignGrade(student1, 101, 87);
    assignGrade(student2, 101, 78);

    console.log('Средняя оценка Анны по JS:', calculateStudentAverage(student1, 101));
    console.log('Статистика курса JS:', getCourseStats(course1, students));
