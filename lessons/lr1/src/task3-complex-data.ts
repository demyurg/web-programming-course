...
interface Student {
  ...
  grades: {
    [courseId: number]: { score: number; date: Date }[];
  };
}
...
// Универсальный хелпер для создания объекта с ID
function createEntity<T extends { id: number }>(entity: T): T {
  return entity;
}
...
function getCourseStats(course: Course, students: Student[]): CourseStats {
  ...
  const averageGrade =
    allGrades.length > 0 ? allGrades.reduce((sum, score) => sum + score, 0) / allGrades.length : 0;

  const studentsWithGrades = enrolledStudents.filter(
    s => s.grades[course.id] && s.grades[course.id]!.length > 0
  ).length;

  const completionRate = enrolledStudents.length > 0 ? (studentsWithGrades / enrolledStudents.length) * 100 : 0;
