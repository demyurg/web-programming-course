...
// Утилита для кеширования (избежали конфликта с DOM Cache API)
...
// Repository с generics и ограничением
...
console.log('=== Тестирование GenericCache ===');
const cache = new GenericCache<string, { name: string; age: number }>();
cache.set('user:1', { name: 'Анна', age: 25 });
console.log('Получили из кеша:', cache.get('user:1'));

console.log('\n=== Тестирование фильтрации и маппинга ===');
const numArray = [1, 2, 3, 4, 5];
const evenNumbers = filterArray(numArray, n => n % 2 === 0);
const doubled = mapArray(numArray, n => n * 2);
...
const userRepo = new Repository<RepoUser>();
