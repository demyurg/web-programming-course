...
const Task2 = observer(() => {
  // Деструктурируем для удобства, MobX будет отслеживать доступ к этим полям
  const {
    gameStatus,
    currentQuestion,
    selectedAnswer,
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-500 to-teal-600 p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm text-gray-600">
              Вопрос {gameStore.currentQuestionIndex + 1} из {questions.length}
            </span>
        ...
        <div className="mt-4 bg-white/20 backdrop-blur-sm rounded-lg p-4 text-white">
          <p className="text-sm">
            <strong>MobX:</strong> Компонент автоматически обновляется при изменении observable полей в store.
          </p>
        </div>
      </div>
    </div>
