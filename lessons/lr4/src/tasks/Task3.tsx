...
const Task3 = () => {
  // Используем селекторы для подписки на конкретные части состояния
  const theme = useUIStore((state) => state.theme);
  const soundEnabled = useUIStore((state) => state.soundEnabled);

  // Получаем actions (они не вызывают ре-рендер при изменении стейта)
        ...
        <div className={`${cardBg} rounded-2xl shadow-2xl p-8 transition-colors duration-300`}>
          ...
          <div className={`mt-6 rounded-lg p-4 ${theme === 'light' ? 'bg-orange-50' : 'bg-gray-700'}`}>
            <p className={`text-sm ${theme === 'light' ? 'text-orange-800' : 'text-gray-300'}`}>
              <strong>Task 3:</strong> UIStore реализован с persist middleware.
              Попробуйте перезагрузить страницу — настройки сохранятся.
            </p>
          </div>
        </div>
      </div>
