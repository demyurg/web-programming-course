...
### 0.4 Сброс кеша Vite (при необходимости)

Если после изменений в конфигурации (например, в `tailwind.config.ts`) стили не обновляются, очистите кеш Vite:
...
Также можно использовать флаг `--force` при запуске:
