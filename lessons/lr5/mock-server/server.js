...
server.get('/api/auth/github', (req, res) => {
  ...
  res.json({
    ...
    mockLoginUrl:
      'http://localhost:3000/api/auth/github/callback?code=mock_code',
  })
})
...
server.put('/api/mode', (req, res) => {
  ...
  db.set('mode', {
    mode,
    battleConfig: mode === 'battle' ? battleConfig : null,
  }).write()
...
server.post('/api/sessions', (req, res) => {
  ...
  if (selectedQuestions.length === 0) {
    return sendError(
      ...
      400,
      'BadRequest',
      'No questions available with these filters',
    )
  }
...
server.get('/api/admin/users', (req, res) => {
  ...
  const usersWithStats = users.map(user => {
    ...
    const avgScore =
      completed.length > 0
        ? completed.reduce(
            (sum, s) => sum + (s.currentScore / s.maxScore) * 100,
            0,
          ) / completed.length
        : 0
...
server.post('/api/admin/answers/:answerId/grade', (req, res) => {
  ...
  const session = db
    .get('sessions')
    .find({ sessionId: answer.value().sessionId })
  if (session.value()) {
    session
      .assign({
...
server.listen(PORT, () => {
  ...
  console.log(``)
  console.log(
    `🔑 To get auth token: GET /api/auth/github/callback?code=mock_code`,
  )
