...
const Task4 = observer(() => {
  ...
  const selectedAnswers = gameStore.selectedAnswers; // TODO: заменить на gameStore.selectedAnswer
  ...
  const handleStartGame = () => {
    createSession.mutate(
      ...
      {
        onSuccess: (response: any) => {  // <-- Добавь тип any (или импорт из generated, см. ниже)
          setSessionId(response.sessionId);
          gameStore.setQuestionsFromAPI(response.questions || []);  // <-- Добавь fallback []
        },
        onError: (error) => {
      ...
      submitAnswer.mutate(
        {
          sessionId,
          data: {
            answers: selectedAnswers.map(idx => idx.toString())  // Фикс
          } as any
        },
        {
          onSuccess: (response: any) => {  // <-- Добавь any для response, аналогично
            if ('pointsEarned' in response) {
              const isCorrect = response.status === 'correct';
              gameStore.updateAnswerResult(isCorrect, response.pointsEarned);
           }
        ...
        <div className={`${cardBg} rounded-2xl shadow-2xl p-6 transition-colors duration-300`}>
          ...
          <div className="space-y-3">
            {currentQuestion.options.map((option, index) => {
              ...
              const isCorrect = currentQuestion.correctAnswer.includes(index);
