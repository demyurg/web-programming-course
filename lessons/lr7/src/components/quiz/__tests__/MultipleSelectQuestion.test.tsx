...
describe('MultipleSelectQuestion', () => {
  ...
  it('should disable all buttons when disabled', () => {
    render(<MultipleSelectQuestion {...defaultProps} disabled />) // ренедерим компонент с неактивными кнопками

    const buttons = screen.getAllByRole('button') // записываем всевозможные значения кнопок
    buttons.forEach(button => { //перебираем все кнопки, если все кнопки выключены то тест  пройден
